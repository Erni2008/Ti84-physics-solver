...
menu_result_t menu_select_formula(const category_def_t *category, uint8_t *selected_index) {
    const char *labels[MAX_FORMULAS_PER_CATEGORY];
    uint8_t i = 0;
    ...
    for (i = 0; i < category->formula_count; ++i) {
        labels[i] = category->formulas[i]->expression;
    }

    return run_menu(category->name, labels, category->formula_count, selected_index);
}
